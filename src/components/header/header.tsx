    ...
    <>
      <header className={styles.header}>
        <div >
          ...
          <Link href="http://localhost:3000/profile" className={styles.logoLink}>
            ...
            {session ?
              (<>
                <div className={styles.burgerButton} onClick={() => setOpen(!open)}>{open ? <MdOutlineClose size={24} /> : <BiMenu size={24}/>}        
                    ...
                    <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute rounded-lg md:static md:z-auto z-[-10] left-4 w-11/12 md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in bg-white md:dark:bg-componentBG dark:bg-darkModeButtonBg ${open ? 'top-14 opacity-100' : 'top-[-500px] md:opacity-100 opacity-0'}`}>
