...
export default function Users(props: any) {
  ...
  const filter = () => {
    if (users.length > 0) {
      ...
      return users.filter((u: any) => u.firstName?.toLowerCase().includes(text) || u.lastName?.toLowerCase().includes(text) || u.email?.toLowerCase().includes(text) || u.phone?.toLowerCase().includes(text) || (u.firstName?.toLowerCase() + " " + u.lastName?.toLowerCase()).includes(text) || (u.lastName?.toLowerCase() + " " + u.firstName?.toLowerCase()).includes(text));
    }
    else return [];
  }
  ...
  return (
    <div className={styles.container}>
            ...
            {filter().map((user: any, index: number) => {
              return (
                <tr key={user.id} className={styles.tr}>
                  ...
                  <th className={styles.th}>{local.admin.table.role}</th>
            ...
            {filter().map((user: any, index: number) => {
              return (
                <tr key={user.id} className={styles.tr}>
                  ...
                  <td className={styles.td}><UserStatus user={user} getUsers={getUsers} local={local}/></td>
                  <td className={styles.td}><UserRole user={user} getUsers={getUsers} local={local}/></td>
                  <td className={styles.td}>
                    <div className='flex gap-3'>
                      ...
                      {new Date(user.blockingTime) < new Date() ?
                        <button className={styles.button_red_BG} onClick={() => { updateUserBlockingTime(user.id, time, number) }}>{local.admin.table.action.block}</button>
                        :
                        <button className={styles.button_blue_BG} onClick={() => { updateUserBlockingTime(user.id, time, 0) }}>{local.admin.table.action.unblock}</button>}
                    </div>
                  </td>
                </tr>
              );
      ...
      <SelectedItem array={filter()} isSelected={isSelected} setIsSelected={setIsSelected} selectedIndex={selectedIndex} setSelectedIndex={setSelectedIndex}>
        <UserInfo user={filter()[selectedIndex]} getUsers={getUsers} local={local} />
      </SelectedItem>
    </div>
  )
}
